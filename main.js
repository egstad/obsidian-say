...
Content-Type: ${audioBlob.type || "audio/webm"}\r
\r
`
  );
...
`
  ...
  const epilogue = encoder.encode(`--${boundary}--\r
`);
  const total = preamble.byteLength + audioBuffer.byteLength + modelPart.byteLength + formatPart.byteLength + epilogue.byteLength;
  const body = new Uint8Array(total);
  let offset = 0;
  body.set(preamble, offset);
  offset += preamble.byteLength;
  body.set(new Uint8Array(audioBuffer), offset);
  offset += audioBuffer.byteLength;
  body.set(modelPart, offset);
  offset += modelPart.byteLength;
  body.set(formatPart, offset);
  offset += formatPart.byteLength;
